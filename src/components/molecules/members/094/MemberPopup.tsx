...
const MemberPopup = ({ isOpen, onClose }: MemberPopupProps) => {
  ...
  return (
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto px-4 pt-28 pb-8 sm:pt-32 bg-black/70 backdrop-blur-md group/modal">
      ...
      {/* MAIN CONTAINER: Identity Card Layer */}
      <div
        className="relative z-10 max-h-[calc(100vh-9rem)] w-full max-w-[720px] overflow-hidden p-6 sm:max-h-[calc(100vh-10rem)] sm:p-8 rounded-2xl border-2 text-white shadow-2xl transition-all duration-300 ease-out select-none
        bg-[#0a0a0a] border-[#ff4500]/50 interactive-fire card-container group overflow-y-auto"
        onMouseMove={(e) => {
          // Logic Tracking Kursor Api berlayer 3
      ...
      >
        ...
        {/* BACKGROUND AURA (4 Warna Palet: Merah, Putih, Abu, Hitam) */}
        ...
        <div className="absolute inset-0 pointer-events-none z-[1] overflow-hidden">
          {[
            { t: '🍀', x: '8%',  y: '12%', s: 70,  r: '-15deg', d: '3.2s' },
            { t: '🍀', x: '82%', y: '65%', s: 110, r: '20deg',  d: '4.5s' },
            { t: '⛧',  x: '72%', y: '8%',  s: 80,  r: '10deg',  d: '3.8s' },
            { t: '†',  x: '15%', y: '72%', s: 90,  r: '-8deg',  d: '5s'   },
            { t: '💀', x: '88%', y: '30%', s: 60,  r: '5deg',   d: '2.8s' },
            { t: 'D',  x: '40%', y: '85%', s: 160, r: '0deg',   d: '6s',  font: 'Cinzel' },
            { t: '✦',  x: '55%', y: '20%', s: 45,  r: '30deg',  d: '4s'   },
            { t: '◈',  x: '25%', y: '45%', s: 40,  r: '-20deg', d: '3.5s' },
          ].map((sym, i) => (
            <span
              key={i}
              className="bg-symbol-drift absolute"
              style={{
                left: sym.x,
                top: sym.y,
                fontSize: sym.s,
                fontFamily: sym.font ?? 'inherit',
                color: ['#dc2626','#ff4500','#888','#fff'][i % 4],
                opacity: 0.06,
                animationDuration: sym.d,
                animationDelay: `${i * 0.4}s`,
                ['--rot' as string]: sym.r,
                transform: `rotate(${sym.r})`,
              }}
            >{sym.t}</span>
          ))}
        </div>
        ...
        {/* Floating fire particles on identity card */}
        ...
        {/* TOMBOL CLOSE (Z-index 60 agar selalu bisa ditutup jika user stuck) */}
        <button
          type="button"
          aria-label="Close member detail"
          onClick={onClose}
          className="absolute top-4 right-4 flex h-9 w-9 items-center justify-center rounded-full border text-xl leading-none z-[60] transition-all duration-300
          border-[#ff4500]/50 text-[#ff4500] bg-black/60 backdrop-blur-sm
          hover:bg-[#ff4500] hover:text-white hover:border-[#ff4500] hover:scale-110 hover:shadow-[0_0_15px_#ff4500]"
        >
          ×
        </button>
        ...
        <div className="relative z-10 flex justify-between items-center mb-6 border-b border-[#ff4500]/30 pb-2 text-[11px] font-mono tracking-widest text-[#ff4500]">
          ...
          <div className="text-gray-300">ANTI-MAGIC : <span className="text-white font-bold">ACTIVE</span></div>
        </div>
